...
def run_evaluation(dataset_path: str, label: str, question_type: str = None):
    with open(dataset_path, encoding="utf-8") as f:
        questions = json.load(f)["questions"]

    #questions = questions[:20]  # Para pruebas rápidas, luego quitar este límite
    #random.seed(123)
    #questions = random.sample(questions, min(1, len(questions)))  # Evaluar un máximo de 20 preguntas aleatorias

    if question_type:
        questions = [q for q in questions if q.get("question_type") == question_type]
        print(f"[{label}] Filtered to {len(questions)} questions of type '{question_type}'")

    if not questions:
        print(f"[{label}] ⚠️ No questions found for type '{question_type}'")
        return {m: float("nan") for m in ["context_recall", "context_precision", "faithfulness", "answer_relevancy"]}
    ...
    for i, q in enumerate(questions):
        ...
        gt = q["ground_truth"].strip()
        
        # if len(gt) < 20:
        #     gt = f"{q['question'].rstrip('?')} es {gt}"
        ...
        results.append({
            ...
            "ground_truth": gt#q["ground_truth"].strip()
        })
    ...
    return {
        metric: np.nanmean(scores[metric]) if isinstance(scores[metric], list) else scores[metric]
...
if __name__ == "__main__":
    ...
    scores_baseline = run_evaluation("dataset_baseline.json", "Baseline dataset")
    ...
    summary_df.to_csv("evaluation_summary_verb_2.csv", index=False, encoding="utf-8")
    print("\nSummary saved to evaluation_summary_verb_2.csv")